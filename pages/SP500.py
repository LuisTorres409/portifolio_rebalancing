...
st.title("Backtest de Estratégias de Rebalanceamento - S&P 500")
st.write("Este aplicativo realiza um backtest de várias estratégias de rebalanceamento de portfólio com base em ativos do S&P 500.")

os.makedirs("data", exist_ok=True)
os.makedirs("results_sp500", exist_ok=True)
...
@st.cache_data
def load_data():
    index_data_path = "data/index_data_sp500.pkl"
    assets_data_path = "data/assets_data_sp500.pkl"
    current_date = datetime(2025, 4, 4)

    def download_and_save_data():
        index_data_daily = yf.download('SPY', start='2000-01-01', end=current_date.strftime('%Y-%m-%d'), interval='1d')
        index_data = index_data_daily[['Close']][index_data_daily.index.weekday == 4].dropna()

        if isinstance(index_data.columns, pd.MultiIndex):
            index_data.columns = [''.join(col) if isinstance(col, tuple) else col for col in index_data.columns]
        if 'CloseSPY' in index_data.columns:
            index_data = index_data.rename(columns={'CloseSPY': 'Close'})
        elif 'Close' not in index_data.columns:
            raise KeyError(f"Coluna 'Close' não encontrada em index_data. Colunas disponíveis: {index_data.columns.tolist()}")

        assets_list = ['DIS', 'VZ', 'HD', 'KO', 'PG', 'CVX', 'XOM', 'AXP', 'JPM', 'JNJ', 'MRK', 'BA', 'SPY', 'AAPL', 'IBM']
        assets_data_daily = yf.download(assets_list, start='2000-01-01', end=current_date.strftime('%Y-%m-%d'), interval='1d')['Close']
        if isinstance(assets_data_daily.columns, pd.MultiIndex):
            assets_data_daily.columns = [''.join(col) if isinstance(col, tuple) else col for col in assets_data_daily.columns]
...
for strategy_name, (model, param_grid, lambda_val) in strategies.items():
    with st.spinner(f"Calculando {strategy_name}..."):
        results_df, weights_history, prob_hist = portfolio_rebalance(
            ...
            common_index=common_index, test_start_date='2010-01-01', index_name='sp500'
        )
        ...
        if i + j < len(sorted_strategies):
            ...
            with col:
                with st.expander(f"{strategy_name}", expanded=False):
                    ...
                    metrics = calculate_metrics(results[strategy_name])
                    baseline_metrics = calculate_metrics(results['S&P 500'])
                    with col2:
...
selected_strategy = st.selectbox(
    ...
    valid_strategies,
    key='weights_history_strategy_sp500'
)
